...
class SettingsDialog(ctk.CTkToplevel):
    def __init__(self, parent, settings: dict, on_save_callback: Callable):
        ...
        # 保存按钮
        save_btn = ctk.CTkButton(self, text="💾 保存设置", command=self.save_settings, fg_color="#4CAF50")
...
class TaskItem(ctk.CTkFrame):
    def __init__(self, master, task: Task, on_retry_callback=None, on_delete_callback=None, on_open_callback=None):
...
class BilibiliSubtitleApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        log_label = ctk.CTkLabel(log_frame, text="运行日志:", anchor="w")
        log_label.grid(row=0, column=0, sticky="w", padx=10, pady=(5, 0))
        
        self.log_textbox = ctk.CTkTextbox(log_frame, font=("Consolas", 10), state="disabled")
        self.log_textbox.grid(row=1, column=0, sticky="nsew", padx=10, pady=5)
    
    def on_add_click(self):
        """处理添加按钮点击 - 带防重复保护"""
        # 如果正在处理，忽略此次点击
        if self._input_processing:
            return
        ...
        if self.input_callback:
            try:
                ...
                self.add_log(f"❌ 添加失败: {e}")
        
        # 1.5秒后解锁
        self.after(1500, self._reset_input_lock)
    ...
    def add_task(self, task: Task):
        task_item = TaskItem(
            self.task_container,
            task,
            on_retry_callback=self._on_task_retry,
            on_delete_callback=self._on_task_delete,
            on_open_callback=self._on_task_open
        )
        task_item.grid(row=len(self.task_frames), column=0, sticky="ew", padx=5, pady=2)
        self.task_frames[task.id] = task_item
    ...
    def _reorder_tasks(self):
        for idx, (task_id, frame) in enumerate(self.task_frames.items()):
    ...
    def _on_task_open(self, task_id: str):
        ...
        try:
            if task.subtitle_path:
                target_dir = os.path.dirname(task.subtitle_path)
            elif task.audio_path:
                target_dir = os.path.dirname(task.audio_path)
            else:
                target_dir = str(config.subtitles_dir)

            os.makedirs(target_dir, exist_ok=True)
            os.startfile(target_dir)
            self.add_log(f"📂 已打开目录: {target_dir}")
        except Exception as exc:
            self.add_log(f"❌ 打开目录失败: {exc}")
